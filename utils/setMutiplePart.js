...
const setMutiplePartDB = async(account, accountType, insertValues, updateValues) => {
  let fields = ["account_name", "account_type", "account_value", "account_desc"]
  // 判断mysql数据库里是否存在该数据
...
const setMutiplePart = async(xmlJson) => {
  ...
  if(commconfig.systemManager.indexOf(xmlJson.FromUserName) < 0 ) {
  ...
  let accountTypes = ["daTaoKeAppKey", "daTaoKeAppSecret", "JDunionId", "PDDpid", "DdxapiKey", "returnRate", "waimaiUrl", "JTKapikey", "JTKsid", "hasMoulds", "JTKpubId"]
  ...
  let insertMap = {account_name:xmlJson.ToUserName, account_type:acountKey, account_value:acountValue, account_desc:desc}
  console.log('insertMap===>', insertMap)
  let updateValues = [xmlJson.ToUserName, acountKey, acountValue, desc]
